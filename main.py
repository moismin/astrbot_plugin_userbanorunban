...
class CJWatchdog(Star):
    def __init__(self, context: Context) -> None:
        ...
        self._timeouts: Dict[str, int] = self._state.setdefault("timeouts", {})
        self._bans: Dict[str, bool] = self._state.setdefault("bans", {})
        self._init_task: Optional[asyncio.Task] = asyncio.create_task(self._check_timeouts_once())
    ...
    def _resolve_data_dir(self) -> Path:
        candidates = []
        for attr in ("data_dir", "data_path", "DATA_PATH"):
    ...
    async def _check_timeouts_once(self) -> None:
        self._log_info(
            "Init-timeout scan scheduled. sleep=%s seconds ctx=%s",
            INIT_TIMEOUT_SECONDS,
            SYSTEM_CONTEXT,
        )
        await asyncio.sleep(INIT_TIMEOUT_SECONDS)
    ...
    async def _call_manager(self, manager: Any, method: str, *args: Any) -> bool:
        ...
        try:
            ...
            self._log_warning("Manager method %s failed: %s", method, exc)
            return False
    ...
    async def _disable_via_manager(self, name: str, enable: bool) -> bool:
        ...
        if not manager:
            return False
        if enable:
            methods = ("enable_star", "activate_star", "enable", "activate", "set_star_activated")
        else:
            methods = (
                "disable_star",
                "deactivate_star",
        ...
        for method in methods:
            if method == "set_star_activated":
                if await self._call_manager(manager, method, name, enable):
                    return True
            else:
